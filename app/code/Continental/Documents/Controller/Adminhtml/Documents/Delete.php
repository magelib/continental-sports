<?php


namespace Continental\Documents\Controller\Adminhtml\Documents;

class Delete extends \Continental\Documents\Controller\Adminhtml\Documents
{

    /**
     * Delete action
     *
     * @return \Magento\Framework\Controller\ResultInterface
     */
    public function execute()
    {
        /** @var \Magento\Backend\Model\View\Result\Redirect $resultRedirect */
        $resultRedirect = $this->resultRedirectFactory->create();
        // check if we know what should be deleted
        $id = $this->getRequest()->getParam('documents_id');
        if ($id) {
            try {
                // init model and delete
                $model = $this->_objectManager->create('Continental\Documents\Model\Documents');
                $model->load($id);
                $model->delete();
                // display success message
                $this->messageManager->addSuccessMessage(__('You deleted the Documents.'));
                // go to grid
                return $resultRedirect->setPath('*/*/');
            } catch (\Exception $e) {
                // display error message
                $this->messageManager->addErrorMessage($e->getMessage());
                // go back to edit form
                return $resultRedirect->setPath('*/*/edit', ['documents_id' => $id]);
            }
        }
        // display error message
        $this->messageManager->addErrorMessage(__('We can\'t find a Documents to delete.'));
        // go to grid
        return $resultRedirect->setPath('*/*/');
    }
}
